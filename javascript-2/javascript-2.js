...
foods.forEach(el => el.calories=(el.carbs*4)+(el.protein*4)+(el.fat*9))
...
blueProducts = saleProducts.filter(el => el.color.includes("blue") ? true : false)
...
let orderTotal = blueProducts.reduce((acc, el) => {return el.price+acc}, 0);
...
const person = {
  name: 'David',
  age: 28,
  jobs: ['student', 'Ecological Restorationist', 'Lab Technician'],
  birthday: function () {
    return this.age += 1;
  },
  favorites: {
